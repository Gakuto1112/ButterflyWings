...
Color = {
    ...
    drawBaseTexture = function ()
        ...
        TextureGenerator.addTextureQueue(textures["base"], 1, 56, function(texture, _, y)
            ...
            if Color.TatterState == "SOFT" then
                for _, chunk in ipairs(Color.SoftTatterDrawData[y + 1]) do
                    texture:fill(chunk[1], y, chunk[2], 1, 0, 0, 0, 0)
                    instructionsUsed = instructionsUsed + 60
                end
            elseif Color.TatterState == "HARD" then
                for _, chunk in ipairs(Color.HardTatterDrawData[y + 1]) do
                    texture:fill(chunk[1], y, chunk[2], 1, 0, 0, 0, 0)
                    instructionsUsed = instructionsUsed + 60
                end
            end
    ...
    drawEdgeTexture = function()
        ...
        TextureGenerator.addTextureQueue(textures["edge"], 1, 56, function (texture, _, y)
            local instructionsUsed = y == 1 and 3000 or 0
            for _, chunk in ipairs(edgeDrawData[y + 1]) do
                texture:fill(chunk[1], y, chunk[2], 1, Color.Color[3])
                instructionsUsed = instructionsUsed + 30
            end
            if Color.TatterState == "SOFT" then
                for _, chunk in ipairs(Color.SoftTatterDrawData[y + 1]) do
                    texture:fill(chunk[1], y, chunk[2], 1, 0, 0, 0, 0)
    ...
    drawPatternTexture = function()
        ...
        TextureGenerator.addTextureQueue(textures["pattern"], 1, 56, function (texture, _, y)
            local instructionsUsed = y == 1 and 3000 or 0
            for _, chunk in ipairs(patternDrawData[y + 1]) do
                texture:fill(chunk[1], y, chunk[2], 1, Color.Color[4])
                instructionsUsed = instructionsUsed + 30
            end
            if Color.TatterState == "SOFT" then
                for _, chunk in ipairs(Color.SoftTatterDrawData[y + 1]) do
