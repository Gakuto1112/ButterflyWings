    ...
    drawWingGradation = function ()
        ...
        for y, row in ipairs(drawArea) do
            for _, chunk in ipairs(row) do
                textures["textures.main"]:fill(chunk[1], y - 1, chunk[2], 1, math.map(y, 1, 56, Color.Color[1].x, Color.Color[2].x), math.map(y, 1, 56, Color.Color[1].y, Color.Color[2].y), math.map(y, 1, 56, Color.Color[1].z, Color.Color[2].z), 0.75)
            end
        end
